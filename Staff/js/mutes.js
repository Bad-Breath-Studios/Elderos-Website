/* ============================================================
   ELDEROS STAFF PANEL - MUTES MANAGEMENT
   ============================================================ */
console.log('[Mutes] Loading mutes.js...');
...
const Mutes = {
    ...
    render() {
        const container = document.getElementById('page-mutes');
        if (!container) return;
        ...
        container.innerHTML = `
            <div class="punishment-page">
                ...
                <div class="punishment-filters" id="mutesFilters">
                    ${this._renderFilters()}
                </div>
                <div class="punishment-table-wrapper">
                    <div class="punishment-table" id="mutesTable">
                        <div class="punishment-loading">Loading...</div>
                    </div>
                    <div class="punishment-pagination" id="mutesPagination"></div>
                </div>
            </div>
        `;
        ...
        return `
            <div class="punishment-filter-row">
                <div class="punishment-filter-group">
                    <label>Search</label>
                    <input type="text" id="mutesFilterSearch" class="punishment-input" placeholder="Username, reason..." value="${this._escapeHtml(this._filters.search || '')}">
                </div>
                <div class="punishment-filter-group">
                    <label>Date From</label>
                    <input type="date" id="mutesFilterStartDate" class="punishment-input" value="${this._filters._startDateVal || ''}">
                </div>
                <div class="punishment-filter-group">
                    <label>Date To</label>
                    <input type="date" id="mutesFilterEndDate" class="punishment-input" value="${this._filters._endDateVal || ''}">
                </div>
                <div class="punishment-filter-actions">
                    <button class="punishment-filter-btn" id="mutesFilterApplyBtn">Apply</button>
                    <button class="punishment-filter-clear" id="mutesFilterClearBtn">Clear</button>
                </div>
            </div>
        `;
    },
    ...
    _bindEvents() {
        // Tab switching
        document.querySelectorAll('#page-mutes .punishment-tab').forEach(tab => {
        ...
        // Apply filters
        document.getElementById('mutesFilterApplyBtn')?.addEventListener('click', () => this._applyFilters());

        // Clear filters
        document.getElementById('mutesFilterClearBtn')?.addEventListener('click', () => {
            this._filters = {};
            this._page = 1;
            this.render();
            this.load();
        ...
        // Enter key
        document.querySelectorAll('#page-mutes .punishment-input').forEach(input => {
    ...
    _applyFilters() {
        const search = document.getElementById('mutesFilterSearch')?.value?.trim();
        const startDateVal = document.getElementById('mutesFilterStartDate')?.value;
        const endDateVal = document.getElementById('mutesFilterEndDate')?.value;
    ...
    async load() {
        ...
        const tableEl = document.getElementById('mutesTable');
        ...
        try {
            ...
            console.error('[Mutes] Load error:', error);
            if (tableEl) {
                tableEl.innerHTML = `
                    <div class="punishment-empty">
                        <div class="punishment-empty-text">Failed to load mutes: ${this._escapeHtml(error.message)}</div>
                    </div>
                `;
            }
        } finally {
    ...
    _renderTable() {
        ...
        if (this._punishments.length === 0) {
            tableEl.innerHTML = `
                <div class="punishment-empty">
                    <div class="punishment-empty-text">No mutes found</div>
                </div>
            `;
            return;
        }
        ...
        tableEl.innerHTML = `
            <table class="punishment-data-table">
                ...
                <tbody>
                    ${this._punishments.map(p => this._renderMuteRow(p, showRevoke, isHistory)).join('')}
                </tbody>
            </table>
        `;
    ...
    _renderMuteRow(p, showRevoke, isHistory) {
        ...
        return `
            <tr>
                ...
                ${showRevoke && isActive ? `<td><button class="punishment-revoke-btn" data-punishment-id="${p.id}">Unmute</button></td>` : showRevoke ? '<td></td>' : ''}
            </tr>
        `;
    },
    ...
    _canRevoke() {
        return typeof Auth !== 'undefined' && Auth.hasPermission(CONFIG.PERMISSIONS.UNMUTE_PLAYER);
    },
    ...
    _showRevokeModal(punishmentId) {
        ...
        modal.innerHTML = `
            <div class="punishment-revoke-modal-content">
                <div class="punishment-revoke-modal-title">Unmute Player</div>
                <div class="punishment-revoke-modal-subtitle">Provide a reason for unmuting this player (min 10 characters)</div>
                <textarea id="revokeReasonInput" placeholder="Reason for unmuting..."></textarea>
                ...
                <div class="punishment-revoke-modal-actions">
                    ...
                    <button class="punishment-revoke-modal-confirm" id="revokeConfirmBtn" disabled>Unmute</button>
                </div>
            </div>
        `;
        ...
        confirmBtn.addEventListener('click', async () => {
            ...
            confirmBtn.textContent = 'Unmuting...';

            try {
                await API.punishments.revoke(punishmentId, reason);
                modal.remove();
                if (typeof Toast !== 'undefined') {
                    Toast.success('Player unmuted successfully');
                }
                this.load();
            } catch (error) {
                console.error('[Mutes] Revoke error:', error);
                if (typeof Toast !== 'undefined') {
                    Toast.error('Failed to unmute: ' + error.message);
                }
                confirmBtn.disabled = false;
                confirmBtn.textContent = 'Unmute';
            }
        });
    },

    _renderPagination() {
        const el = document.getElementById('mutesPagination');
        if (!el) return;
